...
parser.add_argument('--te_batch_size', type=int, default=1024, help='test batch size')
...
def main():
    ...
    for epo_idx in range(args.epoch):
       ...
       # cudnn.benchmark = True
        ...
        net.load_state_dict( {k.replace('module.',''):v for k,v in torch.load(args.parameter_path + "parameter_3_e20.pkl", map_location=args.device).items()} )
        net.to(args.device)

        ### test
        test("te", epo_idx, net, dp, args, log, 1)
        # 0: all
        # 1: covid
        # 2: NC
        # 3: nsclc
