...
parser.add_argument('--print_step', type=int, default=300, help='')
...
def worker(wid, ngpus_per_node, args):
    ...
    n_paths = dp.n_paths
    n_diss = dp.n_diss
    log.info("Node num: n_chems->{}, n_genes->{}, n_paths->{}, n_diss->{}".format(n_chems, n_genes, n_paths, n_diss))
    ...
    ### tr sampler, tr_loader
    tr_edges, rc_lens_list = dp.load_edges("tr_edges.pkl")
    ...
    train(net, optimizer, criterion, tr_sampler, tr_loader, dp, args, log)


### train
def train(net, optimizer, criterion, tr_sampler, tr_loader, dp, args, log):
    log.info(">"*90)
